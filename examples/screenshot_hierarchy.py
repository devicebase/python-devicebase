...
def main() -> None:
    ...
    with DeviceBaseHttpClient() as client:
        ...
        screenshot = client.get_screenshot(serialno)
        ...
        # GET /v1/screenshot/{serialno}; the cross-family /v1/screen above is the
        # one every platform shares.
        attachment = client.download_screenshot(serialno)
        with open(f"{serialno}_screenshot.jpg", "wb") as handle:
        ...
        hierarchy = client.dump_hierarchy(serialno)
        print(f"Hierarchy payload: {len(str(hierarchy.data))} chars")

        # On a browser or computer serialno, dump_hierarchy is not available —
