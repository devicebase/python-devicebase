...
def main() -> None:
    ...
    print(f"Driving desktop {serial} ({os_type})")
    ...
    with DeviceBaseHttpClient() as client:
        ...
        print(f"permissions: {client.computer_permissions(serial).payload}")
        ...
        # --- Mouse ---
        client.computer_move(serial, centre_x, centre_y)
        client.computer_click(serial, centre_x, centre_y)
        client.computer_click(serial, centre_x, centre_y, button="right")
        client.computer_double_click(serial, centre_x, centre_y)
        # duration is in seconds; omitted, the driver picks its own.
        client.computer_long_click(serial, centre_x, centre_y, duration=2)
        client.computer_drag(serial, 100, 100, centre_x, centre_y)
        client.computer_scroll(serial, "down", amount=3)

        print(f"pointer now at {client.computer_position(serial).payload}")

        # --- Keyboard ---
        client.computer_type_text(serial, "你好, world")
        client.computer_press(serial, "Enter")
        client.computer_hotkey(serial, [modifier, "c"])

        # --- Apps ---
        client.computer_launch_app(serial, "Safari")  # a macOS example; name the
        ...
        client.computer_wait(serial, 2000)  # milliseconds
        ...
        result = client.computer_bash(serial, "uname -a", timeout_seconds=30)
