...
A serialno is discovered, never guessed, and it is only meaningful within one
platform family — a mobile serialno cannot drive a browser endpoint.
...
def main() -> None:
    ...
    for device in list_devices(limit=20):
        print(
            f"  {device.serialno:32} {device.type:10} {device.os_type:12} "
    ...
    for device in list_devices(device_type="mobile", limit=5):
        print(f"  {device.serialno} ({device.os_type} {device.os_version})")
    ...
    if devices:
        print(f"\nFirst mobile: {devices[0].serialno}")
